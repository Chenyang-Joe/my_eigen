...
class CholmodBase : public SparseSolverBase<Derived>
{
  ...
  public:
    ...
    {
      // std::cout<<"Cholmod factorization happened here"<<std::endl;
      ...
      cholmod_sparse A = viewAsCholmod(matrix.template selfadjointView<UpLo>());
      // std::cout << "stype " << A.stype << std::endl;
      
      // Generate unique filename with index if file exists
      // std::string base_path = "/mnt/hdd1/chenyang/benchmark_data/larger_matrix_exp/new_mat_bin/3D_golf_ball_vanilla_CholmodSupernodalLLT_4/cholmod_write";
      // std::string file_extension = ".mtx";
      // int index = 0;
      // std::string file_name = base_path + "_" + std::to_string(index) + file_extension;


      // while (FILE* test_file = fopen(file_name.c_str(), "r")) {
      //     index++;
      //     fclose(test_file);  // File exists, close it
      //     file_name = base_path + "_" + std::to_string(index) + file_extension;
      // }
      
      // FILE* file = fopen(file_name.c_str(), "w");
      // if (file != nullptr) {
      //     cholmod_write_sparse(file, &A, 0, 0, &m_cholmod);
      //     fclose(file);
      //     std::cout << "Matrix A written to: " << file_name << std::endl;
      // } else {
      //     std::cout << "Error: Could not open file for writing matrix A: " << file_name << std::endl;
      // }
